...
final class UserLinkService extends BaseService
{
    ...
    public function destroyMany(int $productId, Request $request): ?\stdClass
    {
        $this->withAuth();
        ...
        $response = $this->api
            ...
            ->setBaseUrl($this->host)
            ->setPrefix($this->prefix)
            ->delete("/products/{$productId}/users/destroy-many", $request->all())
            ->getObject();
    ...
    public function reset(int $id): ?\stdClass
    {
        $this->withAuth();
